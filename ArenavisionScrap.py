...
def obtenerUrlsAcestream():
	...
	for index in range(len(urlsPaginasArenavision)):
		...
		urlsAcestream.append(substring)

def obtenerTablaHorarios():
	global horariosArenavision
	global tablaHorarios

	indiceHorarios = horariosArenavision.find("<table")
	substringHorarios = horariosArenavision[indiceHorarios:]
	indiceHorarios = substringHorarios.find("</table>");
	substringHorarios = substringHorarios[:indiceHorarios]
	substringHorarios = substringHorarios + '</table>'

	tablaHorarios = substringHorarios

def generarHTML():
	global cssArenavision
	global tablaHorarios
	global urlsAcestream

	htmlHorarios = open('horarios.html', 'w')
	htmlHorarios.write(cssArenavision)

	for index in range(len(urlsAcestream)):
		htmlHorarios.write(" | <a style='font-size:30px' href='"+urlsAcestream[index]+"'>"+str(index+1)+"</a> ")

	htmlHorarios.write(tablaHorarios)
	htmlHorarios.close()
...
obtenerMenuArenavision()
obtenerEnlaceHorariosArenavision()
obtenerPaginaHorariosArenavision()
obtenerUrlsPaginasArenavision()
obtenerUrlsAcestream()
obtenerTablaHorarios()
generarHTML()